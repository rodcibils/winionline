...
public class Partido {
	...
	public ArrayList<negocio.Partido> getAllAmistosos(int id) throws ClassNotFoundException, SQLException
	{
		PreparedStatement stmt;
		Connection conn = ConnectionManager.getInstance().getConnection();
		
		String query = "SELECT p.fecha, j_uno.nombre, j_uno.apodo, j_dos.nombre, "
				+ "j_dos.apodo, r_uno.goles, r_dos.goles "
				+ "FROM partidos AS p "
				+ "INNER JOIN solicitudes AS s ON s.id = p.solicitud "
				+ "INNER JOIN usuarios AS j_uno ON j_uno.id = s.jugador_uno "
				+ "INNER JOIN usuarios AS j_dos ON j_dos.id = s.jugador_dos "
				+ "INNER JOIN resultados AS r_uno ON (r_uno.id_jugador = s.jugador_uno AND "
				+ "r_uno.id_partido = p.id) "
				+ "INNER JOIN resultados AS r_dos ON (r_dos.id_jugador = s.jugador_dos AND "
				+ "r_dos.id_partido = p.id) "
				+ "WHERE p.estado = ? AND s.liga IS NULL AND (j_uno.id = ? OR j_dos.id = ?) "
				+ "ORDER BY p.fecha";
	...
	public ArrayList<negocio.Partido> getPartidosUsuarioLiga(int idUsuario, int idLiga) throws ClassNotFoundException, SQLException
	{
		PreparedStatement stmt;
		Connection conn = ConnectionManager.getInstance().getConnection();
		
		String query = "SELECT p.fecha, j_uno.nombre, j_uno.apodo, j_dos.nombre, "
				+ "j_dos.apodo, r_uno.goles, r_dos.goles, s.liga "
				+ "FROM partidos AS p "
				+ "INNER JOIN solicitudes AS s ON s.id = p.solicitud "
				+ "INNER JOIN usuarios AS j_uno ON j_uno.id = s.jugador_uno "
				+ "INNER JOIN usuarios AS j_dos ON j_dos.id = s.jugador_dos "
				...
				+ "INNER JOIN resultados AS r_dos ON (r_dos.id_partido = p.id AND "
				+ "r_dos.id_jugador = s.jugador_dos) "
				+ "WHERE p.estado = ? AND s.liga = ? AND (j_uno.id = ? OR j_dos.id = ?) "
				+ "ORDER BY p.fecha";
